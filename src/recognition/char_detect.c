...
l_list *r_find_char_image(image *input, l_list *y_lines) {
    ...
    /* Get image length */
    unsigned long list_len = list_length(y_lines);
    /* Found a pixel in x ? */
    unsigned char found_pixel = 0;
    /* Found a char ? */
    unsigned char found_char = 0;
    /* Current y line cursor */
    unsigned long current_cursor = 0;
    /* Current x position */
    unsigned long current_x = 0;
    /* No need to go further */ 
    unsigned long max_x = input->width;
    ...
    while (current_cursor < list_len) {
        ...
        while (current_x < max_x){
            /* Find a pixel */
            found_pixel = r_find_pixel_vertical(input, current_x, 
                    list_get_value(y_lines, current_cursor),
            ...
            if (found_char && !found_pixel) {
                ...
                list_add(&char_pos,list_get_value(y_lines, current_cursor + 1));
