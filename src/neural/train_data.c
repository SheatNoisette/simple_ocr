...
int nbLines(FILE *file)
{
    ...
    rewind(file); // The function moves the file position indicator to the
                  // beginning of the specified stream, while also clearing the
...
{
    Data data = { new2d(rows, nbInput), new2d(rows, nbOutput), nbInput,
                  nbOutput, rows };
    return data;
}
