...
*/
matrix *crop_image_export_matrix(image *source, l_list *char_position,
                                 int char_number)
{
    // Extract image
    image *temp_image =
        crop_image_at(source, list_get_value(char_position, char_number),
...
*/
char *get_text_ocr(image *input_img, l_list *text_stucture,
                   l_list *char_position, Neurone neural_network)
{
    // Cursor of the current caracter selected detected
    int current_char = 0;

    // Get length of the list
    size_t list_size = list_length(text_stucture);
    ...
    for (size_t cursor = 0; cursor < list_size; cursor++)
    {
        ...
        {
            // Crop character
            matrix_cropped = crop_image_export_matrix(input_img, char_position,
                                                      current_char * 4);
            // Give it to NN
            *(out_string + cursor) =
                get_char_from_nn(matrix_cropped, neural_network);
...
** Run OCR
*/
char *ocr_run(char *file, unsigned char visual, unsigned char otsu,
              unsigned char force_sdl, char *filters, char *train_weight)
{
    ...
    output_text =
        get_text_ocr(ocr_image, text_structure, char_pos, neural_network);
