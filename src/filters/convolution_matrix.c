...
image* extend_border(image* input, unsigned int width)
{
    for (unsigned int i = width/2; i < input->width; i++) {
        ...
        int red_low = get_pixel(input,C_RED,i,input->height -1 -(width/2));
        int green_low = get_pixel(input,C_GREEN,i,input->height -1 -(width/2));
        int blue_low = get_pixel(input,C_BLUE,i,input->height -1 -(width/2));
    ...
    for (unsigned int i = 0; i < input->height; i++) {
        ...
        int red_right = get_pixel(input,C_RED,input->width -1 -(width/2),i);
        int green_right = get_pixel(input,C_GREEN,input->width -1 -(width/2),i);
        int blue_right = get_pixel(input,C_BLUE,input->width -1 -(width/2),i);
