...
*/
matrix* create_pixel_matrix
(image* input, unsigned int x, unsigned int y, int width)
{
    /* Iterators */
    unsigned int i;
    unsigned int j;
...
*/
int convolution_matrix_multiply
(matrix* pixels, matrix* convolution_mask, int active)
{
    /* Iterators */
    int i;
    int j;
...
image* extend_border(image* input, unsigned int width)
{
    ...
    for (i = width/2; i < input->width; i++) {

        /* calculate the upper border pixel color */
        red_up = get_pixel(input,C_RED,i,width/2);
        green_up = get_pixel(input,C_GREEN,i,width/2);
        blue_up = get_pixel(input,C_BLUE,i,width/2);

        /* calculate the lower border pixel color */
        red_low = get_pixel(input,C_RED,i,input->height -1 -(width/2));
        green_low = get_pixel(input,C_GREEN,i,input->height -1 -(width/2));
        blue_low = get_pixel(input,C_BLUE,i,input->height -1 -(width/2));
        ...
        for (j = 0; j < width/2; j++) {
            ...
            set_pixel(input,i,input->height - 1 - j,red_low,green_low,blue_low);
        }
    }
    for (i = 0; i < input->height; i++) {

        /* calculate the left border pixel color */
        red_left = get_pixel(input,C_RED,width/2,i);
        green_left = get_pixel(input,C_GREEN,width/2,i);
        blue_left = get_pixel(input,C_BLUE,width/2,i);

        /* calculate the right border pixel color */
        red_right = get_pixel(input,C_RED,input->width -1 -(width/2),i);
        green_right = get_pixel(input,C_GREEN,input->width -1 -(width/2),i);
        blue_right = get_pixel(input,C_BLUE,input->width -1 -(width/2),i);
...
*/
image* convolution_matrix_filter
(image* input, matrix* convolution_mask,int active)
{
    /* Iterators */
    unsigned int i;
    unsigned int j;
    ...
    for (i = convolution_mask->width/2; 
        i < input->width - 1 - convolution_mask->width/2; 
        i++) {

        for (j = convolution_mask->width/2; 
            j < input->height - 1 - convolution_mask->width/2; 
            j++) {

            new_pixel = convolution_matrix_multiply(

                                        create_pixel_matrix(
                                                    input,
                                                    i,
                                                    j,
                                                    convolution_mask->width),
