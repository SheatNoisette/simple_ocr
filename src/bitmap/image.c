...
image *sdl_surface_to_image(SDL_Surface *input)
{
    ...
    for (unsigned int y = 0; y < image_height; y++)
    {
        ...
        {
            SDL_GetRGB(getpixel(input, x, y), pixel_format, &red, &green,
                       &blue);
...
SDL_Surface *image_to_sdl_surface(image *input)
{
    ...
    sdl_surf = SDL_CreateRGBSurfaceWithFormat(0, input->width, input->height, 8,
    ...
    for (unsigned int y = 0; y < image_height; y++)
    {
        for (unsigned int x = 0; x < image_width; x++)
        {
            ...
            // Create a SDL_pixel
            sdl_pixel = SDL_MapRGB(sdl_surf->format, (Uint8)red, (Uint8)green,
                                   (Uint8)blue);
...
*/
unsigned char get_pixel(image *input, color_i col, unsigned int x,
                        unsigned int y)
{
...
*/
unsigned char get_pixel_safe(image *input, color_i col, unsigned int x,
                             unsigned int y)
{
...
** Write pixel
*/
void set_pixel(image *input, unsigned int x, unsigned int y, unsigned char r,
...
*/
void set_pixel_safe(image *input, unsigned int x, unsigned int y,
