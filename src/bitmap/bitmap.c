...
typedef struct {
        ...
        uint32_t offset;           //the offset between the start of the file and the pixel description
...
image *pixelize(image *image_to_change,char *start, int height,int width)
{
        for(int y = height-1;y>=0;y--) {
                for (int x = 0; x < width; x++) {
                        set_pixel(image_to_change,x,y,*(start + 2),*(start + 1),*(start));
                        start = start + 3;
                }
...
image *bitmaploader(char* path) {
        ...
        if (he->filetype != 0x4d42) {
                if (he->filetype == 0x4142 || he->filetype == 0x4943 || he->filetype == 0x4349 || he->filetype == 0x5043 || he->filetype == 0x5450)
                {
                    //return a warning to inform that it can't be used by our loader and will be processed by SDL
                    fprintf(stdout, "[bitmap] Warning: Type error not BM signature but still a bitmap falling back to SDL\n");
        ...
        /*
        ** Check if the image has a certain dib dib = 40 is the only one for the moment
